# Samba(UNIX系列 與 微軟Windows 溝通) 筆記

```
用來讓UNIX系列的作業系統與微軟Windows作業系統的SMB/CIFS（Server Message Block/Common Internet File System）網路協定做連結的自由軟體。
第三版不僅可存取及分享SMB的資料夾及印表機，本身還可以整合入Windows Server的網域，扮演為網域控制站（Domain Controller）以及加入Active Directory成員。
簡而言之，此軟體在Windows與UNIX系列作業系統之間搭起一座橋梁，讓兩者的資源可互通有無。
```

## 參考資料

[Samba - wiki](https://zh.wikipedia.org/zh-tw/Samba)
...
[設定 SAMBA 伺服器- 鳥哥](https://linux.vbird.org/linux_server/redhat6.1/linux_27samba.php)

### 配置文件相關

[smb.conf — Samba 套件的配置文件](https://www.samba.org/samba/docs/current/man-html/smb.conf.5.html)
...
# 查看掛載內容 可當作連線檢測
mount -t cifs //192.168.1.1/mount_name /target_path

# 使用主機使用者帳密掛載
mount -t cifs -o username="username",password="password" //192.168.1.1/mount_name /target_path
```
...
# 開機自動掛載
vim /etc/fstab
//192.168.1.1/mount_name /target_path cifs username=sambauser,password=pass 0 0
```