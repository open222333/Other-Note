...
| 模式 | 說明 | 預設輸出 |
|------|------|---------|
| `all` | 備份所有使用者資料庫（系統 DB 自動排除） | 單一 `all_databases-YYYYMMDD.sql.gz` |
...
```
兩階段還原腳本：
  階段一：fix_mysql84.py 修正 SQL 相容性（字元集、欄位寬度、collation 等），multiprocessing 加速
...
[階段一] fix_mysql84.py 修正 SQL → <input>_mysql84.sql（進度條顯示）
         .sql.gz 由 Python 直接串流解壓縮，不產生暫存檔
         使用 --workers $(nproc) 平行處理
  ↓
清空 swap（使用中超過 1GB 則跳過）
  ↓
...
```sh
bash mysqlrestore_and_fix_mysql84_timer.sh [--skip-fix] <input.sql[.gz]> [database]
...
```sh
bash mysqlrestore_and_fix_mysql84_timer.sh all_databases.sql.gz          # 修正 + 匯入全部資料庫
bash mysqlrestore_and_fix_mysql84_timer.sh dump.sql.gz mydb              # 修正 + 匯入指定資料庫
bash mysqlrestore_and_fix_mysql84_timer.sh --skip-fix all_databases_mysql84.sql  # 跳過修正直接匯入
```
...
- `.sql.gz` 由 fix_mysql84.py 直接串流解壓縮，不產生暫存 `.sql` 檔
- 修正後輸出自動命名為 `<原檔名>_mysql84.sql`
- fix_mysql84.py 修正內容：整數寬度移除、utf8/utf8mb3→utf8mb4、ROW_FORMAT 移除、collation 統一、charset_add_collate 等
- fix_mysql84.py 使用 multiprocessing（`--workers $(nproc)`），以 32MB 為單位切 chunk，避免 OOM
- swap 清空保護：使用中超過 1GB 自動跳過，避免觸發 OOM killer
- exit code `0` = 兩階段全部成功；`1` = 任一失敗
