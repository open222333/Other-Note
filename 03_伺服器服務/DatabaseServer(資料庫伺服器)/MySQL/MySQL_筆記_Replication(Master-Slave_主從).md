...
```
MySQL Replication 三種模式說明
非同步（Async）— 傳統 Master-Slave
Master 寫入後不等 Slave 確認，直接回應給應用程式。速度最快，但 Master 突然掛掉時，Slave 可能還沒收到最新資料，有少量資料遺失風險。
半同步（Semi-sync）
Master 寫入後，至少等 1 台 Slave 確認收到 binlog 才回應。比非同步安全，速度稍慢。Master 掛掉時資料不會遺失（至少一台 Slave 已收到）。
GTID 模式
這不是同步策略，而是一種追蹤 transaction 的方式，可以搭配上面兩種使用。每筆交易有唯一 ID，Slave 斷線重連後能自動找到正確位置，不需要手動指定 binlog 檔名和位置，管理更方便。
```
...
- [MySQL Replication 和 MySQL Group Replication](#mysql-replication-和-mysql-group-replication)
  ...
  - [參考資料](#參考資料)
    ...
    - [相關筆記](#相關筆記)
- [指令](#指令)
  - [版本語法差異對照（5.7 / 8.0 vs 8.4）](#版本語法差異對照57--80-vs-84)
- [配置](#配置)
  - [MySQL 5.7 / 8.0](#mysql-57--80)
  - [MySQL 8.4](#mysql-84)
  - [MySQL 8.4 非同步 + GTID 模式，1 Master + 2 Slave 安裝步驟](#mysql-84-非同步--gtid-模式1-master--2-slave-安裝步驟)
    - [環境假設](#環境假設)
    - [一、Master 設定](#一master-設定)
      - [1. 編輯設定檔](#1-編輯設定檔)
      - [2. 重啟 MySQL](#2-重啟-mysql)
      - [3. 建立 Replication 帳號](#3-建立-replication-帳號)
      - [4. 確認 GTID 已啟用](#4-確認-gtid-已啟用)
    - [二、Slave 設定（兩台都執行）](#二slave-設定兩台都執行)
    - [1. 編輯設定檔](#1-編輯設定檔-1)
    - [2. 重啟 MySQL](#2-重啟-mysql-1)
    - [3. 設定連線到 Master](#3-設定連線到-master)
    - [4. 啟動 Replication](#4-啟動-replication)
    - [5. 確認同步狀態](#5-確認同步狀態)
    - [三、驗證同步](#三驗證同步)
    - [常見錯誤處理](#常見錯誤處理)
    - [Replica\_IO\_Running: No](#replica_io_running-no)
    - [Replica\_SQL\_Running: No](#replica_sql_running-no)
    - [連線被拒](#連線被拒)
...
[MySQL 8.4版本 文檔 - Replication](https://dev.mysql.com/doc/refman/8.4/en/replication.html)

[MySQL 8.4版本 文檔 - 使用搜尋查詢](https://dev.mysql.com/doc/refman/8.4/en/)
...
## 版本語法差異對照（5.7 / 8.0 vs 8.4）

MySQL 8.4 將 `SLAVE` 相關語法全面改為 `REPLICA`，`MASTER` 改為 `SOURCE`。
...
> MySQL 8.4 移除了所有 `SLAVE` / `MASTER` 舊語法，必須使用 `REPLICA` / `SOURCE`。
> 預設認證方式為 `caching_sha2_password`，建立複製帳號時需注意。
...
```sql
-- 建立複製帳號（8.4 預設 caching_sha2_password）
CREATE USER 'replication'@'192.168.%' IDENTIFIED BY 'StrongPassword!';
GRANT REPLICATION SLAVE ON *.* TO 'replication'@'192.168.%';
FLUSH PRIVILEGES;

-- 查看 Source 狀態（記錄 File 和 Position）
...
| mysql-bin.000001 |     2584 |              |                  |                   |
+------------------+----------+--------------+------------------+-------------------+
-- 記錄 {$source_log_file} = mysql-bin.000001
-- 記錄 {$source_log_pos} = 2584
```
...
```sql
-- 確認 server_id 與 read_only
SHOW VARIABLES LIKE 'server_id%';
SHOW VARIABLES LIKE 'read_only%';
SHOW VARIABLES LIKE 'super_read_only%';
```

`備份 Source 資料並恢復到 Replica`
...
```bash
# xtrabackup 備份（Source 上執行）
xtrabackup --user={$username} --password={$password} --backup --target-dir={$backup_path}

# 傳送到 Replica
rsync -Pr {$backup_path} root@{$replica_ip}:{$backup_path}
```
...
# Replica 上恢復資料
systemctl stop mysqld
cd /var/lib && cp -r ./mysql ./mysql.bak
rm -rf /var/lib/mysql/*
...
CHANGE REPLICATION SOURCE TO
  ...
  -- caching_sha2_password 需要此設定（或改用 SSL）
...
```sql
-- 使用 GTID 時不需要指定 SOURCE_LOG_FILE / SOURCE_LOG_POS
CHANGE REPLICATION SOURCE TO
  SOURCE_HOST='{$source_ip}',
  SOURCE_PORT=3306,
  SOURCE_USER='replication',
...
## MySQL 8.4 非同步 + GTID 模式，1 Master + 2 Slave 安裝步驟
...
CREATE USER 'replication'@'%' IDENTIFIED WITH caching_sha2_password BY '強密碼';
...
> `SOURCE_AUTO_POSITION=1` 是 GTID 模式的關鍵，會自動找同步位置，不需要手動指定 binlog 檔名。
...
通常是網路不通或帳號密碼錯誤，檢查 `Last_IO_Error` 欄位：
...
### 附錄：MySQL 8.4 Replication 相關指令對照
...
| `relay-log-slave` | `relay-log-replica` |
