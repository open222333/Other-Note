...
- [安裝](#安裝)
  - [CentOS7](#centos7)
- [指令](#指令)
  - [openssl 指令](#openssl-指令)
    - [生成密鑰](#生成密鑰)
  - [ssh指令](#ssh指令)
  - [scp指令 (傳遞檔案)](#scp指令-傳遞檔案)
    - [rsync 與 scp 區別](#rsync-與-scp-區別)
  - [ssh-copy-id 指令](#ssh-copy-id-指令)
...
# 查看憑證資訊（日期、Subject、Issuer 等）
openssl x509 -in certificate.crt -text -noout

# 測試 SSL/TLS 連線
openssl s_client -connect host:443

# 產生隨機 hex 字串（32 bytes）
openssl rand -hex 32

# 產生隨機 base64 字串（32 bytes）
...
# 產生 16 bytes 二進制 key 並寫入檔案
openssl rand 16 > /path/to/key.key
...
```bash
# 複製公鑰至遠端主機（自動寫入 ~/.ssh/authorized_keys）
